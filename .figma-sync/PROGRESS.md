...
## Last completed unit: G44 — Services (308:4492) rebuilt against measured bounding boxes. Nine defects, the worst being that the image column sat on the wrong side of all four blocks (an RTL `order` mistake, same category as G40), plus 96px block rhythm where the frame is 200, a 50/50 grid where the frame is 604|135|505.3, the numeral's 145px line box collapsed to leading-1 (which shifted the whole copy stack ~40px against the photograph), 16px image corners where the frame says 8, a CSS border standing in for the hand-drawn title rule, in-flow sparks that are absolute decorations in the frame, a 1248-wide mesh that should be 1200 inset 24, and an undersized ring centred on a glyph the substitute font sets 43px narrower. Persian copy reseeded verbatim from the frame's text runs. Verified rendered-vs-frame with Playwright bounding boxes: every box now lands within 1px except the hand-placed rings (<=6px, inside the frame's own scatter). (This line had been stale at G38 while GAPS ran to G43.)
## Next unit: Two site-wide questions G44 opened, both needing a second frame sampled before anyone acts: (a) `.section-first` may be 8px short — this frame puts the page-header eyebrow at y=188, the utility lands at 180 (overridden locally on Services only); (b) the `.eyebrow` marker may sit ~6px high site-wide, and `.eyebrow--dot` hardcodes an ink rim where the service feature dots stroke White. Still not value-checked at this tier: Home's services/orbit and testimonials (component-level rigor from G25/G32 trusted, not re-verified), Legal x2, Work index (structure)/Work show, Insights index/show (article prose/meta chips), 404, TestimonialCard, ArticleMeta, FilterChips, ClientLogo. Roughly 1 in 3 components checked so far has had a real, previously-unknown defect; Services had nine.
