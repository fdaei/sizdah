...
## Last completed unit: G45 + G46. G45 closed both site-wide questions G44 left open by reading all seven eyebrow-marker instances instead of one: `.section-first` is correct at 180 (Services is a genuine 188 outlier, already overridden locally) and the marker is correctly centred — Figma reports a *rotated* node's x/y as its CENTRE, and misreading that as the bounding-box top-left is what made it look 5.66px off. The rim, though, was really wrong: the code had generalised `309:4757` (2px Black/800), the one instance that agrees with nothing else, across the whole site; it is 1px Yellow/50 on all four dark-ground instances, 1px Black/800 on the cream band, and 2px White on the service feature bullets. Also removed the dead `square` marker and fixed the 48px eyebrow gap on the testimonials and insights headings. G46 then found that `responsive.spec.ts` had been measuring the page *before Vue mounts* — every no-overflow guarantee it ever made was vacuous. Fixing it surfaced three real overflows (WhyUsGrid 147px at 1024-1279, Contact below 400px, Home at 320px), all fixed. Suite: 137 passed, 0 failed.
## Next unit: The value-check tier still owes: Home's services/orbit and testimonials bodies (component-level rigor from G25/G32 trusted, not re-verified), Legal x2, Work index/show, Insights index/show, 404, and TestimonialCard / ArticleMeta / FilterChips / ClientLogo. Their docblocks carry measured values from earlier passes, so these are re-verifications rather than unknowns. Two environment items are outstanding and need a human with root: `pdo_sqlite` is absent so the Pest suite cannot run at all, and WebKit's system libs are absent so `mobile-390` cannot launch.
